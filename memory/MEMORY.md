...
§
GitHub：hansv0704/hermes-tools。多電腦同步：程式碼用 Git pull，記憶用 MEGA 每30分鐘 cron 雙向 auto sync（sync_memory.py v2 — 比較 mtime 自動判斷 push/pull）。新電腦一鍵安裝：curl -o setup.bat https://raw.githubusercontent.com/hansv0704/hermes-tools/main/一鍵安裝.bat && setup.bat。一鍵安裝會自動複製記憶到 alice + default 雙 profile、設定 MEGA_SYNC_DIR、建立記憶同步 cron。用 hermes -p alice（不用 default）。
§
